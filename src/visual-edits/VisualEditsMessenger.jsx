...
export default function HoverReceiver() {
  ...
  useEffect(() => {
    if (isVisualEditMode && !styleElementRef.current) {
      ...
      style.textContent = `
        ...
        [contenteditable="true"]::-moz-selection { background-color: rgba(59, 130, 246, 0.3); }
        [contenteditable="true"] [contenteditable="false"] { user-select: none !important; -webkit-user-select: none !important; -moz-user-select: none !important; -ms-user-select: none !important; opacity: 0.7 !important; cursor: default !important; }
        [data-orchids-protected="true"] { user-select: none !important; -webkit-user-select: none !important; -moz-user-select: none !important; -ms-user-select: none !important; }
      `;
      ...
      if (e.data?.type === "ORCHIDS_STYLE_UPDATE") {
        ...
        candidates.forEach((el) => {
          if (el.tagName.toLowerCase() === "img") {
            ...
            if (oldSrc && normalizeImageSrc(oldSrc) === norm) element = img;
          }
        });
        if (!element) return;
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        if (parent) {
          const parentRect = parent.getBoundingClientRect();
          const parentStyles = window.getComputedStyle(parent);
          const parentInnerWidth = parentRect.width - (parseFloat(parentStyles.paddingLeft) || 0) - (parseFloat(parentStyles.paddingRight) || 0);
          const parentInnerHeight = parentRect.height - (parseFloat(parentStyles.paddingTop) || 0) - (parseFloat(parentStyles.paddingBottom) || 0);
        ...
        const styles = { width: widthValue, height: heightValue };
        if (hasMaxWidth) styles.maxWidth = widthValue;
        if (hasMaxHeight) styles.maxHeight = heightValue;
        const msg = { type: CHANNEL, msg: "STYLE_BLUR", id: focusedElementId, styles, filePath: "", line: 0, column: 0, className: element.getAttribute("class") || "" };
        const orchidsId = element.getAttribute("data-orchids-id");
        if (orchidsId) {
          const parsed = parseOrchidsId(orchidsId);
          if (parsed) {
  ...
  const cleanupEditingElement = () => {
    if (editingElementRef.current) {
      ...
      if (element.childElementCount > 0) restoreChildElements(element);
      if (!wasEditableRef.current) element.contentEditable = "false";
    ...
    if (focusedElementRef.current) {
      const handleUpdate = () => {
        ...
        if (focusedElementRef.current && focusedElementId) {
          ...
          if (fBox) {
            const focMsg = { type: CHANNEL, msg: "FOCUS_MOVED", id: focusedElementId, rect: { top: fBox.top, left: fBox.left, width: fBox.width, height: fBox.height } };
  ...
  useEffect(() => {
    function onPointerMove(e) {
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const boxes = [];
        allMatchingElements.forEach((element) => {
          if (element.getAttribute("data-orchids-id") === focusedElementId) return;
          boxes.push(expandBox(element.getBoundingClientRect()));
        });
        setHoverBoxes(boxes);
        if (hitId !== focusedElementId) {
          setHoverBox(expandBox(hit.getBoundingClientRect()));
        } else {
          setHoverBox(null);
        }
        setHoverTag(tagName);
    ...
    function onPointerLeave() {
      if (!isVisualEditModeRef.current) return;
      if (isResizingRef.current) return;
      setHoverBox(null); setHoverBoxes([]); setHoverTag(null);
      flushImageSrcChange();
      lastHitElementRef.current = null; lastHitIdRef.current = null;
      postMessageDedup({ type: CHANNEL, msg: "HIT", id: null, tag: null, rect: null });
    }
    ...
    function onClickCapture(e) {
      ...
      if (hit) {
        ...
        if (isLink || isButton || !isEditable) { e.preventDefault(); e.stopPropagation(); }
        ...
        const allMatchingElements = document.querySelectorAll(`[data-orchids-id="${hitId}"]`);
        const boxes = [];
        allMatchingElements.forEach((element) => { if (element === hit) return; boxes.push(expandBox(element.getBoundingClientRect())); });
        setHoverBoxes(boxes);
        if (boxes.length > 0) setHoverTag(tagName);
        if (hit.tagName.toLowerCase() === "img") focusedImageElementRef.current = hit;
        else focusedImageElementRef.current = null;
        ...
        if (isEditable) {
          ...
          if (hit !== editingElementRef.current) {
            ...
            const createHandlers = (element) => {
              const handleFocus = () => { if (element !== editingElementRef.current) return; handleStyleBlur(element); originalContentRef.current = element.childElementCount > 0 ? extractDirectTextContent(element) : (element.innerText || element.textContent || ""); hasStyleChangesRef.current = false; };
              const handleBlur = () => { if (element !== editingElementRef.current) return; handleStyleBlur(element); handleTextChange(element); };
              const handleInput = () => { if (element !== editingElementRef.current) return; };
        ...
        postMessageDedup({ type: CHANNEL, msg: "ELEMENT_CLICKED", id: hitId, tag: tagName, rect: expandedBox ? { top: expandedBox.top, left: expandedBox.left, width: expandedBox.width, height: expandedBox.height } : { top: 0, left: 0, width: 0, height: 0 }, clickPosition: { x: e.clientX, y: e.clientY }, isEditable, currentStyles: computedStyles, className, src: srcRaw });
    ...
    function onMsg(e) {
      ...
      if (e.data.msg === "PREVIEW_FONT" && "elementId" in e.data) {
        ...
        if (!loadedFontFamilies.current.has(familyKey)) {
          const link = document.createElement("link"); link.rel = "stylesheet"; link.href = `https://fonts.googleapis.com/css2?family=${familyKey}:wght@400&display=swap`; document.head.appendChild(link); loadedFontFamilies.current.add(familyKey);
        }
        element.style.fontFamily = `'${fontFamily}', sans-serif`;
        return;
      }
      if (e.data.msg === "SCROLL" && "dx" in e.data && "dy" in e.data) window.scrollBy(e.data.dx, e.data.dy);
      if (e.data.msg === "VISUAL_EDIT_MODE" && "active" in e.data) {
        ...
        if (!newMode) {
          flushImageSrcChange(); cleanupEditingElement(); focusedImageElementRef.current = null;
          setHoverBox(null); setHoverBoxes([]); setFocusBox(null); setFocusedElementId(null); lastHitElementRef.current = null; focusedElementRef.current = null; hasStyleChangesRef.current = false;
          setHoverTag(null); setFocusTag(null);
          postMessageDedup({ type: CHANNEL, msg: "HIT", id: null, tag: null, rect: null });
        }
      }
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        ...
        allMatchingElements.forEach((element) => {
          const stylesToClear = ["fontSize", "color", "fontWeight", "fontStyle", "textDecoration", "textAlign", "paddingLeft", "paddingRight", "paddingTop", "paddingBottom", "marginLeft", "marginRight", "marginTop", "marginBottom", "backgroundColor", "backgroundImage"];
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          setHoverBoxes(boxes); setHoverTag(boxes.length > 0 ? tagName : null);
        }
      }
    }
    function onScroll() {
  ...
  return (
    <div
      id="orchids-visual-edits-messenger"
      style={{
        position: "fixed",
        top: 0,
        left: 0,
    ...
    >
      {/* Multiple Hover Boxes for same data-orchids-id */}
      {hoverBoxes.map((box, idx) => (
        <div
          key={idx}
          style={{
            position: "absolute",
      ...
      {focusBox && (
        ...
        >
          ...
          <div
            ...
            style={{ position: "absolute", top: -4, left: "50%", transform: "translateX(-50%)", width: 20, height: 8, cursor: "n-resize", pointerEvents: "auto" }}
          />
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "s")}
            style={{ position: "absolute", bottom: -4, left: "50%", transform: "translateX(-50%)", width: 20, height: 8, cursor: "s-resize", pointerEvents: "auto" }}
          />
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "e")}
            style={{ position: "absolute", top: "50%", right: -4, transform: "translateY(-50%)", width: 8, height: 20, cursor: "e-resize", pointerEvents: "auto" }}
          />
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "w")}
            style={{ position: "absolute", top: "50%", left: -4, transform: "translateY(-50%)", width: 8, height: 20, cursor: "w-resize", pointerEvents: "auto" }}
          />
          {/* Corner Handles */}
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "se")}
            style={{ position: "absolute", bottom: -4, right: -4, width: 8, height: 8, backgroundColor: "white", border: "2px solid #2563eb", cursor: "se-resize", pointerEvents: "auto" }}
          />
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "sw")}
            style={{ position: "absolute", bottom: -4, left: -4, width: 8, height: 8, backgroundColor: "white", border: "2px solid #2563eb", cursor: "sw-resize", pointerEvents: "auto" }}
          />
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "ne")}
            style={{ position: "absolute", top: -4, right: -4, width: 8, height: 8, backgroundColor: "white", border: "2px solid #2563eb", cursor: "ne-resize", pointerEvents: "auto" }}
          />
          <div
            className="resize-handle"
            onMouseDown={(e) => handleResizeStart(e, "nw")}
            style={{ position: "absolute", top: -4, left: -4, width: 8, height: 8, backgroundColor: "white", border: "2px solid #2563eb", cursor: "nw-resize", pointerEvents: "auto" }}
          />
        </div>
      )}
    </div>
